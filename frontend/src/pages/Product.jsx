          ...
          <Carousel variant='dark'>
            {productCurrent?.productImgs.map(img => (
              <Carousel.Item interval={2000} key={img}>
                <img className='d-block w-100 img-suggestion' src={resolveImgUrl(img)} alt='Photo one of the Product' onLoad={() => dispatch(setLoader(false))} />
              </Carousel.Item>
            ))}
          </Carousel>
        </article>
        <article>
          ...
          <div className='price-quantity'>
            ...
            <div className='quantity'>
              <p>Quantity</p>
              <button disabled={quantity === 1} onClick={() => setQuantity(quantity - 1)} className='material-symbols-outlined'>
                remove
              </button>
              <span>{quantity}</span>
              <button onClick={() => setQuantity(quantity + 1)} className='material-symbols-outlined'>
                add
              </button>
            </div>
            <p>
              <b>Stock:</b> {productCurrent?.stock}
            </p>
            <button onClick={() => addProductToCart()} className='addToCart-currentProduct'>
              Agregar <span className='material-symbols-outlined'>shopping_cart</span>
            </button>
          </div>
          <p className='detail-product'>{productCurrent?.description}</p>
        </article>
      </section>
      <h3 className='h3-suggestion'>Sugerencias</h3>
        ...
        {productsCategories.map(product => (
          <article className='product-card' key={product.id} onClick={() => productSelected(product.id)}>
            <div className='img-product'>
              <img src={resolveImgUrl(product.productImgs?.[0])} alt='' onLoad={() => dispatch(setLoader(false))} />
            </div>
